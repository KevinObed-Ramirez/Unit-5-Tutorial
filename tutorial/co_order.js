...
window.addEventListener("load", function() {
   ...
   orderForm.elements.orderDate.value = newDate().toDateString();
...
function calcOrder(){
   ...
   var qIndex = orderForm.elements.qtyselectedIndex;
   var quanity = orderForm.elements.qty[qIndex].value;

   //Initial Cost = cost * quanity
   var initialCost = mCost*quanity;
   orderForm.elements.initialCost.value = initialCost;

   //retrieve the cost of the user's protection plan
   var pCost = document.querySelector('input[name="protection"]:checked').value*quanity;
...
function formatNumber(val, decimals) {
   return val.loLocaleString(undefined, {minimunFractionDigits: decimals, maximumFractionDigits: decimals});
